...
async function showProducts(ctx: Ctx, category: Category, page: number, query = "", inStock = false) {
  ...
  const rows = items.map((p) => [inlineButton(`${p.name} — ${p.price.toFixed(2)} PLN`, `product:${p.id}:${code}:${safePage}`)]);
...
composer.callbackQuery(/^catalog:search:(liquids|cartridges|pods|disposables)$/, async (ctx) => { await ctx.answerCallbackQuery(); const c = ctx as CatalogCtx; const code = ctx.match[1] as (typeof categoryCodes)[number]; c.session.catalogCategory = categories[code]; c.session.catalogSearch = true; await ctx.editMessageText(`Введите запрос для категории «${displayCategory(c.session.catalogCategory)}».`, { reply_markup: inlineKeyboard([[inlineButton("Назад", `catalog:cancel-search:${code}`)]]) }); });
composer.callbackQuery(/^catalog:cancel-search:(liquids|cartridges|pods|disposables)$/, async (ctx) => { await ctx.answerCallbackQuery(); const c = ctx as CatalogCtx; c.session.catalogSearch = false; c.session.catalogQuery = undefined; await showProducts(ctx, categories[ctx.match[1] as (typeof categoryCodes)[number]], 0); });
...
composer.callbackQuery(/^product:([^:]+):(liquids|cartridges|pods|disposables):(\d+)$/, async (ctx) => {
  await ctx.answerCallbackQuery();
  const p = await get<Product>(ctx, `product:${ctx.match[1]}`);
  const code = ctx.match[2]; const page = ctx.match[3];
  if (!p || !p.visibility) { await ctx.editMessageText("Этот товар больше недоступен.", { reply_markup: back }); return; }
  const stock = p.stock > 0 ? `В наличии: ${p.stock} шт.` : "Нет в наличии.";
  if (p.photo) await ctx.replyWithPhoto(p.photo);
  const photo = p.photo ? "Фото: добавлено." : "Фото: не добавлено.";
  await ctx.editMessageText(`${p.name}\n${p.description}\nSKU: ${p.id}\nЦена: ${p.price.toFixed(2)} PLN\n${stock}\nАкцизная маркировка: ${p.excise_mark}\n${photo}`, { reply_markup: inlineKeyboard([[inlineButton("Добавить в корзину", `cart:add:${p.id}`)], [inlineButton("Назад", `catalog:category:${code}:${page}`)]]) });
});
